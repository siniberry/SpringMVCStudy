...
VALUES(POSITION.NEXTVAL, '사원', 1000000);
-->> 한 줄 구성
...
AS
SELECT P.POSITIONID, P.POSITIONNAME, MINBASICPAY
     , (SELECT COUNT(*)
        FROM EMPLOYEE
        WHERE POSITIONID=P.POSITIONID) AS DELCHECK
FROM POSITION P;
...
--> EMPLOYEEVIEW 테이블에는 SSN2 이 없으므로 EMPLOYEE 테이블로 쿼리문 작성
SELECT NAME
FROM EMPLOYEE
